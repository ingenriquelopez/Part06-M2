...
// Tu código acá:
let miSpan = document.querySelector('#createdBy');
miSpan.innerHTML+= 'Enrique Lopez';
...
function buildToDos(toDos) {
  ...
  let arrayToDo = toDos.map(function(elemento,indice) {  
     return buildToDo(elemento,indice);
  });
return arrayToDo;
}
...
function displayToDos() {
  ...
  let retornado = buildToDos(toDoItems);
  retornado.forEach(function(elemento) {
...
function addToDo() {
  // Tu código acá:
  let  valueToDo = document.getElementById('toDoInput').value;
  let newToDo = new ToDo(valueToDo);
...
// Tu código acá:
let miAdd = document.getElementById('addButton');

miAdd.addEventListener('click',addToDo);
